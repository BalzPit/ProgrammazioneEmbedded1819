...
import gruppoembedded.pse1819.unipd.project.tensorflowlite.env.BorderedText;
...
import gruppoembedded.pse1819.unipd.project.tensorflowlite.tflite.Classifier;
  ...
  private void recreateClassifier(Classifier.Model model, Classifier.Device device, int numThreads) {
    ...
    if (device == Classifier.Device.GPU && model == Classifier.Model.QUANTIZED) {
      ...
      runOnUiThread( new Runnable() {
        @Override
        public void run() {
          Toast.makeText( context, "GPU does not yet supported quantized models.", Toast.LENGTH_LONG)
                  .show();
        }
      });
      return;
